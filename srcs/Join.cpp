...
/*   Updated: 2024/06/12 18:30:26 by mdouglas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::join(std::vector<std::string> _input_cmd, int _client_fd)
{
    ...
    {
        _response = "461 " + _input_cmd[0] + " JOIN :Not enough parameters" + END;
    ...
    std::cout << RED << "Number of channels: " << _channels.size() << RESET << std::endl; // Imprime o número de canais
    ...
    {
        std::cout << GREEN << "Processing channel: " << _channels[i] << RESET << std::endl; // Imprime o canal atual

        if (!checkChannelName(_channels[i]))
        {
            _response = "JOIN " + _channels[i] + " :Invalid channel name" + END;
            send(_client_fd, _response.c_str(), _response.size(), 0);
            continue ;
        }
        ...
        //quero imprimir os nomes dos canais
        std::cout << YELLOW << "Channel name: " << channelPtr->getName() << RESET << std::endl;
        if (channelPtr->getMode("i") == true)
        {
            if (!channelPtr->checkInvite(_client->getNick()))
            {
                ...
                {
                    std::cout << "Removing from invite list" << std::endl;
